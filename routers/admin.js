...
router.post('/categorias/edit', eAdmin, (req,res)=>{
    ...
    if(erros.length > 0){
        ...
        Categoria.findOne({_id:req.body.id}).then((categoria) =>{
            ...
            req.flash("error_msg", "Erro ao buscar a categoria, tente novamente")
            res.redirect('/admin')
        })
...
router.get('/postagens', eAdmin, (req, res) =>{
    Postagem.find().populate("categoria").sort({data:"desc"}).then((postagens) => {
        ...
        req.flash("error_msg","Houve um erro ao listar as postagems");
        res.redirect('/admin')
    })
...
router.post('/postagens/edit', eAdmin, (req,res)=>{
    ...
    if(req.body.categoria == "0"){
        erros.push({texto:"Categoria invalida, por favor selecione uma categoria"})
    }
    if(erros.length > 0){
        ...
        Postagem.findOne({_id:req.body.id}).then((postagem) =>{
            ...
            req.flash("error_msg", "Erro ao buscar a postagem, tente novamente")
            res.redirect('/admin')
        })
